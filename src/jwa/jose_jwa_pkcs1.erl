...
encrypt_public(PlainText, RSAPublicKey = #jose_rsa_public_key{}, Options) when
    is_list(Options)
->
    Res =
        case proplists:get_value(rsa_padding, Options) of
...
sign(Message, DigestType, RSAPrivateKey = #jose_rsa_private_key{}, Options) when
    is_list(Options)
->
    Res =
        case proplists:get_value(rsa_padding, Options) of
...
verify(Message, DigestType, Signature, RSAPublicKey = #jose_rsa_public_key{}, Options) when
    is_list(Options)
->
    case proplists:get_value(rsa_padding, Options) of
        rsa_pkcs1_pss_padding ->
...
-spec eme_oaep_decode(Hash, EM, Label, K) -> M | error when
    ...
    K :: integer(),
    M :: binary().
eme_oaep_decode(Hash, EM, Label, K) when
    is_function(Hash, 1) andalso
        is_binary(EM) andalso
        is_binary(Label) andalso
        is_integer(K)
->
    HLen = byte_size(Hash(<<>>)),
    LHash = Hash(Label),
...
-spec eme_oaep_encode(Hash, DM, Label, Seed, K) -> {ok, EM} | {error, Reason} when
    Hash :: rsa_hash_fun(),
    DM :: binary(),
    Label :: binary(),
    Seed :: binary(),
    ...
    EM :: binary(),
    Reason :: term().
eme_oaep_encode(Hash, DM, Label, Seed, K) when
    is_function(Hash, 1) andalso
        is_binary(DM) andalso
        is_binary(Label) andalso
        is_binary(Seed) andalso
        is_integer(K)
->
    HLen = byte_size(Hash(<<>>)),
    MLen = byte_size(DM),
...
-spec eme_pkcs1_decode(EM, K) -> M | error when
    EM :: binary(),
    K :: integer(),
    M :: binary().
eme_pkcs1_decode(<<16#00, 16#02, Rest/binary>>, K) when
    is_integer(K)
->
...
-spec eme_pkcs1_encode(DM, K) -> {ok, EM} | {error, Reason} when
    DM :: binary(),
    K :: integer(),
    EM :: binary(),
    Reason :: term().
eme_pkcs1_encode(DM, K) when
    is_binary(DM) andalso
        is_integer(K)
->
    MLen = byte_size(DM),
...
-spec emsa_pkcs1_encode(Hash, Algorithm, Message, EMBits) -> {ok, EM} | {error, Reason} when
    Hash :: rsa_hash_fun(),
    ...
    EM :: binary(),
    Reason :: term().
emsa_pkcs1_encode(Hash, Algorithm, Message, EMBits) when
    is_function(Hash, 1) andalso
        is_binary(Algorithm) andalso
...
-spec emsa_pss_encode(Hash, Message, EMBits) -> {ok, EM} | {error, Reason} when
    Hash :: rsa_hash_fun(),
    Message :: binary(),
    EMBits :: integer(),
    EM :: binary(),
    Reason :: term().
emsa_pss_encode(Hash, Message, EMBits) when
    is_function(Hash, 1) andalso
        is_binary(Message) andalso
        is_integer(EMBits)
->
...
%% See [https://tools.ietf.org/html/rfc3447#section-9.1.1]
-spec emsa_pss_encode(Hash, Message, Salt, EMBits) -> {ok, EM} | {error, Reason} when
    Hash :: rsa_hash_fun(),
    Message :: binary(),
    Salt :: binary() | integer(),
    EMBits :: integer(),
    EM :: binary(),
    Reason :: term().
emsa_pss_encode(Hash, Message, Salt, EMBits) when
    is_function(Hash, 1) andalso
        is_binary(Message) andalso
...
emsa_pss_encode(Hash, Message, -2, EMBits) when
    is_function(Hash, 1) andalso
        is_integer(EMBits)
->
...
emsa_pss_encode(Hash, Message, -1, EMBits) when
    is_function(Hash, 1)
->
    HashLen = byte_size(Hash(<<>>)),
    SaltLen = HashLen,
    emsa_pss_encode(Hash, Message, SaltLen, EMBits);
emsa_pss_encode(Hash, Message, SaltLen, EMBits) when
    is_integer(SaltLen) andalso
        SaltLen >= 0
->
...
-spec emsa_pss_verify(Hash, Message, EM, EMBits) -> boolean() when
    Hash :: rsa_hash_fun(),
    Message :: binary(),
    EM :: binary(),
    EMBits :: integer().
emsa_pss_verify(Hash, Message, EM, EMBits) when
    is_function(Hash, 1) andalso
        is_binary(Message) andalso
        is_binary(EM) andalso
        is_integer(EMBits)
...
%% See [https://tools.ietf.org/html/rfc3447#section-9.1.2]
-spec emsa_pss_verify(Hash, Message, EM, SaltLen, EMBits) -> boolean() when
    Hash :: rsa_hash_fun(),
    Message :: binary(),
    EM :: binary(),
    SaltLen :: integer(),
    EMBits :: integer().
emsa_pss_verify(Hash, Message, EM, SaltLen, EMBits) when
    is_function(Hash, 1) andalso
        is_binary(Message) andalso
        is_integer(SaltLen) andalso
        SaltLen >= 0 andalso
...
emsa_pss_verify(Hash, Message, EM, -2, EMBits) when
    is_function(Hash, 1) andalso
        is_integer(EMBits)
->
    HashLen = byte_size(Hash(<<>>)),
...
emsa_pss_verify(Hash, Message, EM, -1, EMBits) when
    is_function(Hash, 1)
->
    HashLen = byte_size(Hash(<<>>)),
    SaltLen = HashLen,
...
-spec mgf1(Hash, Seed, MaskLen) -> {ok, binary()} | {error, mask_too_long} when
    Hash :: rsa_hash_fun(),
    Seed :: binary(),
    MaskLen :: pos_integer().
mgf1(Hash, Seed, MaskLen) when
    is_function(Hash, 1) andalso
        is_binary(Seed) andalso
        is_integer(MaskLen) andalso
        MaskLen >= 0
->
    HashLen = byte_size(Hash(<<>>)),
...
%% See [https://tools.ietf.org/html/rfc3447#section-7.1.2]
-spec rsaes_oaep_decrypt(Hash, CipherText, RSAPrivateKey) -> PlainText when
    Hash :: rsa_hash_fun(),
    CipherText :: binary(),
    RSAPrivateKey :: rsa_private_key(),
    PlainText :: binary().
rsaes_oaep_decrypt(Hash, CipherText, RSAPrivateKey = #jose_rsa_private_key{}) when
    is_function(Hash, 1) andalso
        is_binary(CipherText)
->
    rsaes_oaep_decrypt(Hash, CipherText, <<>>, RSAPrivateKey);
rsaes_oaep_decrypt(Hash, CipherText, RSAPrivateKey) when
    is_tuple(Hash) orelse
        is_atom(Hash)
->
    HashFun = resolve_hash(Hash),
...
%% See [https://tools.ietf.org/html/rfc3447#section-7.1.2]
-spec rsaes_oaep_decrypt(Hash, CipherText, Label, RSAPrivateKey) -> PlainText when
    Hash :: rsa_hash_fun(),
    CipherText :: binary(),
    Label :: binary(),
    RSAPrivateKey :: rsa_private_key(),
    PlainText :: binary().
rsaes_oaep_decrypt(Hash, CipherText, Label, RSAPrivateKey = #jose_rsa_private_key{n = N}) when
    is_function(Hash, 1) andalso
...
rsaes_oaep_decrypt(Hash, CipherText, Label, RSAPrivateKey) when
    is_tuple(Hash) orelse
        is_atom(Hash)
->
    HashFun = resolve_hash(Hash),
...
%% See [https://tools.ietf.org/html/rfc3447#section-7.1.1]
-spec rsaes_oaep_encrypt(Hash, PlainText, RSAPublicKey) -> CipherText when
    Hash :: rsa_hash_fun(),
    PlainText :: binary(),
    RSAPublicKey :: rsa_public_key(),
    CipherText :: binary().
rsaes_oaep_encrypt(Hash, PlainText, RSAPublicKey = #jose_rsa_public_key{}) when
    is_function(Hash, 1) andalso
        is_binary(PlainText)
->
    rsaes_oaep_encrypt(Hash, PlainText, <<>>, RSAPublicKey);
rsaes_oaep_encrypt(Hash, PlainText, RSAPublicKey) when
    is_tuple(Hash) orelse
        is_atom(Hash)
->
    HashFun = resolve_hash(Hash),
...
%% See [https://tools.ietf.org/html/rfc3447#section-7.1.1]
-spec rsaes_oaep_encrypt(Hash, PlainText, Label, RSAPublicKey) -> CipherText when
    Hash :: rsa_hash_fun(),
    PlainText :: binary(),
    Label :: binary(),
    RSAPublicKey :: rsa_public_key(),
    CipherText :: binary().
rsaes_oaep_encrypt(Hash, PlainText, Label, RSAPublicKey = #jose_rsa_public_key{}) when
    is_function(Hash, 1) andalso
...
rsaes_oaep_encrypt(Hash, PlainText, Label, RSAPublicKey) when
    is_tuple(Hash) orelse
        is_atom(Hash)
->
    HashFun = resolve_hash(Hash),
...
%% See [https://tools.ietf.org/html/rfc3447#section-7.1.1]
-spec rsaes_oaep_encrypt(Hash, PlainText, Label, Seed, RSAPublicKey) -> CipherText when
    Hash :: rsa_hash_fun(),
    PlainText :: binary(),
    Label :: binary(),
    Seed :: binary(),
    RSAPublicKey :: rsa_public_key(),
    CipherText :: binary().
rsaes_oaep_encrypt(Hash, PlainText, Label, Seed, RSAPublicKey = #jose_rsa_public_key{n = N}) when
    is_function(Hash, 1) andalso
        is_binary(PlainText) andalso
        is_binary(Label) andalso
        is_binary(Seed)
...
rsaes_oaep_encrypt(Hash, PlainText, Label, Seed, RSAPublicKey) when
    is_tuple(Hash) orelse
        is_atom(Hash)
->
    HashFun = resolve_hash(Hash),
...
%% See [https://tools.ietf.org/html/rfc3447#section-7.2.2]
-spec rsaes_pkcs1_decrypt(CipherText, RSAPrivateKey) -> PlainText when
    CipherText :: binary(),
    RSAPrivateKey :: rsa_private_key(),
    PlainText :: binary().
rsaes_pkcs1_decrypt(CipherText, RSAPrivateKey = #jose_rsa_private_key{n = N}) when
    is_binary(CipherText)
->
    K = byte_size(N),
...
%% See [https://tools.ietf.org/html/rfc3447#section-7.2.1]
-spec rsaes_pkcs1_encrypt(PlainText, RSAPublicKey) -> CipherText when
    PlainText :: binary(),
    RSAPublicKey :: rsa_public_key(),
    CipherText :: binary().
rsaes_pkcs1_encrypt(PlainText, RSAPublicKey = #jose_rsa_public_key{n = N}) when
    is_binary(PlainText)
->
    MLen = byte_size(PlainText),
    K = byte_size(N),
...
-spec rsassa_pkcs1_sign(Hash, Message, RSAPrivateKey) -> {ok, Signature} | {error, Reason} when
    Hash :: rsa_hash_fun(),
    Message :: binary(),
    RSAPrivateKey :: rsa_private_key(),
    Signature :: binary(),
    Reason :: term().
rsassa_pkcs1_sign(Hash, Message, RSAPrivateKey) when
    is_atom(Hash)
->
    rsassa_pkcs1_sign(Hash, Hash, Message, RSAPrivateKey).

%% See [https://tools.ietf.org/html/rfc3447#section-8.1.1]
-spec rsassa_pkcs1_sign(Hash, Algorithm, Message, RSAPrivateKey) -> {ok, Signature} | {error, Reason} when
    Hash :: rsa_hash_fun(),
    Algorithm :: md5 | sha | sha1 | sha256 | sha384 | sha512 | binary(),
    Message :: binary(),
    RSAPrivateKey :: rsa_private_key(),
    Signature :: binary(),
    Reason :: term().
rsassa_pkcs1_sign(Hash, Algorithm, Message, RSAPrivateKey = #jose_rsa_private_key{n = Modulus}) when
    is_function(Hash, 1) andalso
        (is_atom(Algorithm) orelse is_binary(Algorithm)) andalso
        is_binary(Message)
->
...
rsassa_pkcs1_sign(Hash, Algorithm, Message, RSAPrivateKey = #jose_rsa_private_key{}) when
    is_atom(Hash)
->
    HashFun = resolve_hash(Hash),
...
-spec rsassa_pkcs1_verify(Hash, Message, Signature, RSAPublicKey) -> boolean() when
    Hash :: rsa_hash_fun(),
    Message :: binary(),
    Signature :: binary(),
    RSAPublicKey :: rsa_public_key().
rsassa_pkcs1_verify(Hash, Message, Signature, RSAPublicKey) when
    is_atom(Hash)
->
    rsassa_pkcs1_verify(Hash, Hash, Message, Signature, RSAPublicKey).

%% See [https://tools.ietf.org/html/rfc3447#section-8.2.2]
-spec rsassa_pkcs1_verify(Hash, Algorithm, Message, Signature, RSAPublicKey) -> boolean() when
    Hash :: rsa_hash_fun(),
    Algorithm :: md5 | sha | sha1 | sha256 | sha384 | sha512 | binary(),
    Message :: binary(),
    Signature :: binary(),
    RSAPublicKey :: rsa_public_key().
rsassa_pkcs1_verify(Hash, Algorithm, Message, Signature, RSAPublicKey = #jose_rsa_public_key{n = Modulus}) when
    is_function(Hash, 1) andalso
        is_binary(Message) andalso
        is_binary(Signature)
->
...
rsassa_pkcs1_verify(Hash, Algorithm, Message, Signature, RSAPublicKey = #jose_rsa_public_key{}) when
    is_atom(Hash)
->
    HashFun = resolve_hash(Hash),
...
%% See [https://tools.ietf.org/html/rfc3447#section-8.1.1]
-spec rsassa_pss_sign(Hash, Message, RSAPrivateKey) -> {ok, Signature} | {error, Reason} when
    Hash :: rsa_hash_fun(),
    Message :: binary(),
    RSAPrivateKey :: rsa_private_key(),
    Signature :: binary(),
    Reason :: term().
rsassa_pss_sign(Hash, Message, RSAPrivateKey = #jose_rsa_private_key{n = Modulus}) when
    is_function(Hash, 1) andalso
        is_binary(Message)
->
    ModBits = bit_size(Modulus),
...
rsassa_pss_sign(Hash, Message, RSAPrivateKey = #jose_rsa_private_key{}) when
    is_tuple(Hash) orelse
        is_atom(Hash)
->
    HashFun = resolve_hash(Hash),
...
%% See [https://tools.ietf.org/html/rfc3447#section-8.1.1]
-spec rsassa_pss_sign(Hash, Message, Salt, RSAPrivateKey) -> {ok, Signature} | {error, Reason} when
    Hash :: rsa_hash_fun(),
    Message :: binary(),
    Salt :: binary() | integer(),
    RSAPrivateKey :: rsa_private_key(),
    Signature :: binary(),
    Reason :: term().
rsassa_pss_sign(Hash, Message, Salt, RSAPrivateKey = #jose_rsa_private_key{n = Modulus}) when
    is_function(Hash, 1) andalso
        is_binary(Message) andalso
        (is_binary(Salt) orelse is_integer(Salt))
->
    ModBits = bit_size(Modulus),
...
rsassa_pss_sign(Hash, Message, Salt, RSAPrivateKey = #jose_rsa_private_key{}) when
    is_tuple(Hash) orelse
        is_atom(Hash)
->
    HashFun = resolve_hash(Hash),
...
-spec rsassa_pss_verify(Hash, Message, Signature, RSAPublicKey) -> boolean() when
    Hash :: rsa_hash_fun(),
    Message :: binary(),
    Signature :: binary(),
    RSAPublicKey :: rsa_public_key().
rsassa_pss_verify(Hash, Message, Signature, RSAPublicKey = #jose_rsa_public_key{n = Modulus}) when
    is_function(Hash, 1) andalso
        is_binary(Message) andalso
        is_binary(Signature)
->
...
rsassa_pss_verify(Hash, Message, Signature, RSAPublicKey = #jose_rsa_public_key{}) when
    is_tuple(Hash) orelse
        is_atom(Hash)
->
    HashFun = resolve_hash(Hash),
...
%% See [https://tools.ietf.org/html/rfc3447#section-8.1.2]
-spec rsassa_pss_verify(Hash, Message, Signature, SaltLen, RSAPublicKey) -> boolean() when
    Hash :: rsa_hash_fun(),
    Message :: binary(),
    Signature :: binary(),
    SaltLen :: integer(),
    RSAPublicKey :: rsa_public_key().
rsassa_pss_verify(Hash, Message, Signature, SaltLen, RSAPublicKey = #jose_rsa_public_key{n = Modulus}) when
    is_function(Hash, 1) andalso
        is_binary(Message) andalso
...
rsassa_pss_verify(Hash, Message, Signature, SaltLen, RSAPublicKey = #jose_rsa_public_key{}) when
    is_tuple(Hash) orelse
        is_atom(Hash)
->
    HashFun = resolve_hash(Hash),
