...
block_encrypt(Cipher, Key, PlainText) when
    is_binary(PlainText)
->
    case block_cipher(Cipher) of
        {crypto, BlockCipher} ->
...
block_decrypt(Cipher, Key, IV, CipherText) when
    is_binary(CipherText)
->
    case block_cipher(Cipher) of
        {crypto, BlockCipher} ->
...
block_decrypt(Cipher, Key, IV, {AAD, CipherText, CipherTag}) when
...
block_encrypt(Cipher, Key, IV, PlainText) when
    is_binary(PlainText)
->
    case block_cipher(Cipher) of
        {crypto, BlockCipher} ->
...
block_encrypt(Cipher, Key, IV, {AAD, PlainText}) when
    is_binary(AAD) andalso
        is_binary(PlainText)
->
    case block_cipher(Cipher) of
...
encrypt_public(PlainText, RSAPublicKey = #'RSAPublicKey'{}, Algorithm) when
    is_atom(Algorithm)
->
    {Module, Options} = rsa_crypt(Algorithm),
...
sign(Message, DigestType, RSAPrivateKey = #'RSAPrivateKey'{}, Padding) when
    is_atom(Padding)
->
...
verify(Message, DigestType, Signature, RSAPublicKey = #'RSAPublicKey'{}, Padding) when
    is_atom(Padding)
->
    case rsa_sign(Padding) of
        {Module, undefined} ->
...
constant_time_compare(_, <<>>) ->
    false;
constant_time_compare(A, B) when
    is_binary(A) andalso is_binary(B) andalso
