...
% $= z^{-1} \mod p$, for z != 0
-define(inv(Z), ?math:expmod(Z, ?p - 2, ?p)).
...
% 4 * ?inv(5)
-define(By, 46316835694926478169428394003475163141307993866256225615783033603165251855960).
% xrecover(?By)
-define(Bx, 15112221349535400772501151409588531511454012693041857206046113283949847762202).
% {?Bx, ?By, 1, (?Bx * ?Bx) rem ?p}
-define(B, {?Bx, ?By, 1, 46827403850823179245072216630277197565144205554125654976674165829533817101731}).
...
% ?math:intpow(2, 252) + 27742317777372353535851937790883648493
-define(l, 7237005577332262213973186563042994240857116359379907606001950938285454250989).
...
sign_internal(M, <<Secret:?secretbytes/binary, PK:?publickeybytes/binary>>, PHFlag, C) when
    is_binary(M) andalso
...
verify_internal(Sig, M, _PK = <<_:?publickeybytes/binary>>, PHFlag, C) when
    is_binary(Sig) andalso
        is_binary(M) andalso
        (PHFlag =:= 0 orelse PHFlag =:= 1) andalso
        is_binary(C)
->
