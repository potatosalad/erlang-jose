...
%%%
%%% @end
%%% Created :  08 Jan 2016 by Andrew Bennett <potatosaladx@gmail.com>
%%%-------------------------------------------------------------------
...
shake128(Input, OutputSize) when
    is_binary(Input) andalso
        (is_integer(OutputSize) andalso OutputSize >= 0)
->
...
shake256(Input, OutputSize) when
    is_binary(Input) andalso
        (is_integer(OutputSize) andalso OutputSize >= 0)
->
...
keccak_absorb(RateInBytes, InputBytes, StateBytes, DelimitedSuffix) when
    is_integer(RateInBytes) andalso
        byte_size(InputBytes) >= RateInBytes
->
...
keccak_pad(RateInBytes, BlockSize, StateBytes, DelimitedSuffix) ->
    ...
    State1 =
        case (DelimitedSuffix band 16#80) =/= 0 andalso BlockSize =:= (RateInBytes - 1) of
            false ->
                State0;
            true ->
...
keccak_squeeze(RateInBytes, OutputByteLen, StateBytes, OutputBytes) when
    OutputByteLen > 0
->
