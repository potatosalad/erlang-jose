...
key_decrypt(
    {UEphemeralKey = #jose_jwk{}, VStaticSecretKey = #jose_jwk{}},
    EncryptedKey,
...
key_decrypt(
    {UEphemeralPublicKey = #jose_jwk{}, VStaticSecretKey = #jose_jwk{}},
    EncryptedKey,
    JWEECDHES0 = #jose_jwe_alg_ecdh_es{epk = undefined}
) ->
...
key_decrypt(Z, {ENCModule, ENC, <<>>}, #jose_jwe_alg_ecdh_es{apu = APU, apv = APV, wrap = undefined, bits = undefined}) when
    is_binary(Z)
->
...
key_decrypt(
    ...
    JWEECDHES = #jose_jwe_alg_ecdh_es{apu = APU, apv = APV, wrap = aes_gcm_kw, bits = KeyDataLen, iv = IV, tag = TAG}
) when
    is_binary(Z) andalso
...
key_decrypt(
    Z,
    {_ENCModule, _ENC, EncryptedKey},
    JWEECDHES = #jose_jwe_alg_ecdh_es{apu = APU, apv = APV, wrap = c20p_kw, bits = KeyDataLen, iv = IV, tag = TAG}
) when
    is_binary(Z) andalso
        is_binary(IV) andalso
        is_binary(TAG)
...
key_decrypt(
    Z,
    {_ENCModule, _ENC, EncryptedKey},
    JWEECDHES = #jose_jwe_alg_ecdh_es{apu = APU, apv = APV, wrap = xc20p_kw, bits = KeyDataLen, iv = IV, tag = TAG}
) when
    is_binary(Z) andalso
        is_binary(IV) andalso
        is_binary(TAG)
...
key_encrypt(
    Z, DecryptedKey, JWEECDHES = #jose_jwe_alg_ecdh_es{apu = APU, apv = APV, wrap = c20p_kw, bits = KeyDataLen, iv = IV}
) when
    is_binary(Z) andalso
        is_binary(IV)
->
...
next_cek(
    {VStaticPublicKey = #jose_jwk{}, UEphemeralSecretKey = #jose_jwk{}},
    {ENCModule, ENC},
    JWEECDHES = #jose_jwe_alg_ecdh_es{bits = undefined, epk = UEphemeralPublicKey = #jose_jwk{}}
) ->
    case jose_jwk:thumbprint(UEphemeralSecretKey) =:= jose_jwk:thumbprint(UEphemeralPublicKey) of
        true ->
            Z = jose_jwk:shared_secret(VStaticPublicKey, UEphemeralSecretKey),
...
next_cek(
    {VStaticPublicKey = #jose_jwk{}, UEphemeralSecretKey = #jose_jwk{}},
    {ENCModule, ENC},
    JWEECDHES0 = #jose_jwe_alg_ecdh_es{wrap = undefined, bits = undefined, epk = undefined}
) ->
    UEphemeralPublicKey = jose_jwk:to_public(UEphemeralSecretKey),
    JWEECDHES1 = JWEECDHES0#jose_jwe_alg_ecdh_es{epk = UEphemeralPublicKey},
