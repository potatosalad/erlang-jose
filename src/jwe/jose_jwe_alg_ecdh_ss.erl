...
key_decrypt(
    ...
    EncryptedKey,
    JWEECDHSS = #jose_jwe_alg_ecdh_ss{spk = USenderPublicKey = #jose_jwk{}}
) ->
...
key_decrypt(
    {UStaticPublicKey = #jose_jwk{}, VStaticSecretKey = #jose_jwk{}},
    EncryptedKey,
    JWEECDHSS0 = #jose_jwe_alg_ecdh_ss{spk = undefined}
) ->
...
key_decrypt(VStaticSecretKey = #jose_jwk{}, EncryptedKey, JWEECDHSS = #jose_jwe_alg_ecdh_ss{spk = USenderPublicKey = #jose_jwk{}}) ->
    ...
    key_decrypt(Z, EncryptedKey, JWEECDHSS);
key_decrypt(Z, {ENCModule, ENC, <<>>}, #jose_jwe_alg_ecdh_ss{apu = APU, apv = APV, wrap = undefined, bits = undefined}) when
    is_binary(Z)
->
...
key_decrypt(
    ...
    JWEECDHSS = #jose_jwe_alg_ecdh_ss{apu = APU, apv = APV, wrap = aes_gcm_kw, bits = KeyDataLen, iv = IV, tag = TAG}
) when
    is_binary(Z) andalso
...
key_decrypt(
    Z, {_ENCModule, _ENC, EncryptedKey}, JWEECDHSS = #jose_jwe_alg_ecdh_ss{apu = APU, apv = APV, wrap = aes_kw, bits = KeyDataLen}
) when is_binary(Z) ->
    Algorithm = algorithm(JWEECDHSS),
    SuppPubInfo = <<KeyDataLen:1/unsigned-big-integer-unit:32>>,
    DerivedKey = jose_jwa_concat_kdf:kdf(sha256, Z, {Algorithm, APU, APV, SuppPubInfo}, KeyDataLen),
...
key_decrypt(
    Z,
    {_ENCModule, _ENC, EncryptedKey},
    JWEECDHSS = #jose_jwe_alg_ecdh_ss{apu = APU, apv = APV, wrap = c20p_kw, bits = KeyDataLen, iv = IV, tag = TAG}
) when
    is_binary(Z) andalso
        is_binary(IV) andalso
        is_binary(TAG)
...
key_decrypt(
    Z,
    {_ENCModule, _ENC, EncryptedKey},
    JWEECDHSS = #jose_jwe_alg_ecdh_ss{apu = APU, apv = APV, wrap = xc20p_kw, bits = KeyDataLen, iv = IV, tag = TAG}
) when
    is_binary(Z) andalso
        is_binary(IV) andalso
        is_binary(TAG)
...
key_encrypt(_Key, _DecryptedKey, JWEECDHSS = #jose_jwe_alg_ecdh_ss{wrap = undefined, bits = undefined}) ->
    {<<>>, JWEECDHSS};
key_encrypt(
    {VStaticPublicKey = #jose_jwk{}, UStaticSecretKey = #jose_jwk{}},
    DecryptedKey,
    JWEECDHSS = #jose_jwe_alg_ecdh_ss{spk = USenderPublicKey = #jose_jwk{}}
) ->
...
key_encrypt(
    {VStaticPublicKey = #jose_jwk{}, UStaticSecretKey = #jose_jwk{}},
    DecryptedKey,
    JWEECDHSS0 = #jose_jwe_alg_ecdh_ss{wrap = undefined, bits = undefined, spk = undefined}
) ->
...
key_encrypt(
    Z, DecryptedKey, JWEECDHSS = #jose_jwe_alg_ecdh_ss{apu = APU, apv = APV, wrap = aes_gcm_kw, bits = KeyDataLen, iv = IV}
) when
    is_binary(Z) andalso
        is_binary(IV)
->
...
key_encrypt(Z, DecryptedKey, JWEECDHSS = #jose_jwe_alg_ecdh_ss{apu = APU, apv = APV, wrap = aes_kw, bits = KeyDataLen}) when
    is_binary(Z)
->
    Algorithm = algorithm(JWEECDHSS),
    SuppPubInfo = <<KeyDataLen:1/unsigned-big-integer-unit:32>>,
...
key_encrypt(
    Z, DecryptedKey, JWEECDHSS = #jose_jwe_alg_ecdh_ss{apu = APU, apv = APV, wrap = c20p_kw, bits = KeyDataLen, iv = IV}
) when
    is_binary(Z) andalso
        is_binary(IV)
->
    ...
    {CipherText, JWEECDHSS#jose_jwe_alg_ecdh_ss{tag = CipherTag}};
key_encrypt(
    Z, DecryptedKey, JWEECDHSS = #jose_jwe_alg_ecdh_ss{apu = APU, apv = APV, wrap = xc20p_kw, bits = KeyDataLen, iv = IV}
) when
    is_binary(Z) andalso
        is_binary(IV)
->
...
next_cek(
    {VStaticPublicKey = #jose_jwk{}, UStaticSecretKey = #jose_jwk{}},
    {ENCModule, ENC},
    JWEECDHSS = #jose_jwe_alg_ecdh_ss{bits = undefined, spk = USenderPublicKey = #jose_jwk{}}
) ->
    case jose_jwk:thumbprint(UStaticSecretKey) =:= jose_jwk:thumbprint(USenderPublicKey) of
        true ->
            Z = jose_jwk:shared_secret(VStaticPublicKey, UStaticSecretKey),
...
next_cek(
    #jose_jwk{kty = {KTYModule, KTY}}, {ENC, ENCModule}, JWEECDHSS = #jose_jwe_alg_ecdh_ss{wrap = undefined, bits = undefined}
) ->
    DerivedKey = KTYModule:derive_key(KTY),
    next_cek(DerivedKey, {ENCModule, ENC}, JWEECDHSS);
next_cek(Z, {ENCModule, ENC}, JWEECDHSS = #jose_jwe_alg_ecdh_ss{apu = APU, apv = APV, wrap = undefined, bits = undefined}) when
    is_binary(Z)
->
    Algorithm = ENCModule:algorithm(ENC),
    KeyDataLen = ENCModule:key_bit_size(ENC),
