...
key_decrypt(DerivedKey, {_ENCModule, _ENC, EncryptedKey}, #jose_jwe_alg_c20p_kw{iv = IV, tag = TAG}) when
...
key_encrypt(DerivedKey, DecryptedKey, JWEC20PKW = #jose_jwe_alg_c20p_kw{iv = IV}) when
    is_binary(DerivedKey) andalso
        bit_size(DerivedKey) =:= ?BITS andalso
        is_binary(IV)
->
