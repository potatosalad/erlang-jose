...
generate_key(Seed = << _:?secretbytes/binary >>) ->
	{_PK, SK} = jose_curve25519:ed25519ph_keypair(Seed),
	{SK, #{}};
generate_key({okp, 'Ed25519ph', Seed = << _:?secretbytes/binary >>}) ->
	generate_key(Seed);
generate_key({okp, 'Ed25519ph'}) ->
	{_PK, SK} = jose_curve25519:ed25519ph_keypair(),
	{SK, #{}}.
...
sign(Message, 'Ed25519ph', SK = << _:?secretkeybytes/binary >>) ->
...
verify(Message, 'Ed25519ph', Signature, << _:?secretbytes/binary, PK:?publickeybytes/binary >>) ->
	verify(Message, 'Ed25519ph', Signature, PK);
verify(Message, 'Ed25519ph', Signature, PK = << _:?publickeybytes/binary >>) ->
...
from_okp({'Ed25519ph', SK = << Secret:?secretbytes/binary, PK:?publickeybytes/binary >>}) ->
	case jose_curve25519:ed25519ph_secret_to_public(Secret) of
		PK ->
			{SK, #{}};
		_ ->
			erlang:error(badarg)
