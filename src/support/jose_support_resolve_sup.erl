...
-spec start_child(ReplyTo, ReplyTag, Key, ProviderModules) -> supervisor:startchild_ret()
    when ReplyTo :: pid(), ReplyTag :: reference(), Key :: jose_support:key(), ProviderModules :: [{integer(), module()}].
start_child(ReplyTo, ReplyTag, Key = {Behaviour, {FunctionName, Arity}}, ProviderModules)
        when is_pid(ReplyTo)
