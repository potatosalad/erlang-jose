...
%% @private
graph(Graph, Failures, Providers, Behaviours, SupportModules, ProviderModules) ->
	ok = lists:foreach(fun(SupportModule) ->
			...
			true ->
				try SupportModule:support_info() of
					#{
						...
						lists:foreach(fun({Callback, _CallbackRequirements}) ->
	...
	ok = lists:foreach(fun({Behaviour, {_Stateful, Callbacks}}) ->
		lists:foreach(fun({Callback, CallbackRequirements}) ->
			lists:foreach(fun({RequiredBehaviour, RequiredCallbacks}) ->
				lists:foreach(fun(RequiredCallback) ->
					digraph:add_edge(Graph, {RequiredBehaviour, RequiredCallback}, {Behaviour, Callback})
				end, RequiredCallbacks)
			end, CallbackRequirements)
		end, Callbacks)
	end, ets:tab2list(Behaviours)),
	ok = lists:foreach(fun(ProviderModule) ->
					...
					#{
						...
						case ensure_requirements_loaded(Requirements) of
							true ->
								...
								lists:foreach(fun({Callback = {FunctionName, Arity}, _CallbackRequirements}) ->
									case erlang:function_exported(ProviderModule, FunctionName, Arity) of
										true ->
											true = ets:insert(Providers, {{Behaviour, Callback}, {priority_to_integer(Priority), ProviderModule}}),
											ok;
										false ->
											ok
									end
	...
	NotSupported0 =
		...
		end, maps:new(), digraph:vertices(Graph)),
