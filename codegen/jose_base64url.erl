...
encode(Input, Opts) when ?is_iodata(Input) andalso is_map(Opts) ->
    ...
    H = <<
        <<(encode_pair(V0)):16, (encode_pair(V1)):16, (encode_pair(V2)):16, (encode_pair(V3)):16>>
     || <<V0:12, V1:12, V2:12, V3:12>> <= Head
    >>,
    {T, Pad} =
        case Tail of
...
%% @private
encode_char(V) ->
    jose_base:encode_char_as_tuple_element_call(?INT_TO_B64URL(V)).

-compile({inline, [{encode_pair, 1}]}).
%% @private
encode_pair(V) ->
    jose_base:encode_pair_as_tuple_element_call(?INT_TO_B64URL(V), sensitive).
