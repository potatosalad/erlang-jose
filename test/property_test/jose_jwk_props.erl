                ...
                {<<"ECDH-1PU", _/binary>>, _} ->
                    ?LET(
                        ...
                        begin
                            ...
                            {{ecdh_1pu, VStaticKeypair, UStaticKeypair, UEphemeralKeypair}, JWE, {
                                {VStaticSecretKey, VStaticPublicKey},
                                {UStaticSecretKey, UStaticPublicKey},
                                {UEphemeralSecretKey, UEphemeralPublicKey}
                            }}
                        end
                    );
                {<<"ECDH-ES", _/binary>>, _} ->
                    ?LET(
                        ...
                        begin
                            ...
                            {{ecdh_es, AliceKeypair, BobKeypair}, JWE, {
                                {AlicePrivateJWK, AlicePublicJWK}, {BobPrivateJWK, BobPublicJWK}
                            }}
                        end
                    );
                {<<"A128GCMKW">>, _} ->
...
modulus_size() -> integer(1048, 1280).
% pos_integer().
        ...
        begin
            case {Keys, JWKs} of
                {{ecdh_1pu, _, _, _}, {
                    ...
                    Encrypted = jose_jwk:box_encrypt_ecdh_1pu(
                ...
                {{ecdh_es, _, _}, {{AlicePrivateJWK, _AlicePublicJWK}, {BobPrivateJWK, BobPublicJWK}}} ->
                    ...
                    {PlainText, NewJWE} =:= Decrypted andalso
                        {PlainText, NewJWE} =:= jose_jwk:block_decrypt(CompactEncrypted, BobPrivateJWK);
                {{rsa, _, _}, {PrivateJWK, PublicJWK}} ->
                    ...
                    {PlainText, NewJWE} =:= Decrypted andalso
                        {PlainText, NewJWE} =:= jose_jwk:block_decrypt(CompactEncrypted, PrivateJWK);
                {_, JWK} ->
                    Encrypted = jose_jwk:block_encrypt(PlainText, JWE, JWK),
                    CompactEncrypted = jose_jwe:compact(Encrypted),
                    Decrypted = {_, NewJWE} = jose_jwk:block_decrypt(Encrypted, JWK),
                    {PlainText, NewJWE} =:= Decrypted andalso
                        {PlainText, NewJWE} =:= jose_jwk:block_decrypt(CompactEncrypted, JWK)
            end
        end
    ).
