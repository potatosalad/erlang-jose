...
x448_keypair(Secret) ->
	{PK, Secret} = jose_curve448:x448_keypair(Secret),
...
jwk_map() ->
	?LET({AliceSecret, BobSecret},
		...
		begin
			...
			AlicePrivateJWK = jose_jwk:from_okp({'X448', AlicePrivateKey}),
...
prop_box_encrypt_and_box_decrypt() ->
	?FORALL({{{_, {BobPrivateKey, BobPublicKey}}, AlicePrivateJWK}, PlainText},
		{jwk_gen(), binary()},
		begin
			BobPrivateJWK = jose_jwk:from_okp({'X448', BobPrivateKey}),
			BobPublicJWK = jose_jwk:from_okp({'X448', BobPublicKey}),
