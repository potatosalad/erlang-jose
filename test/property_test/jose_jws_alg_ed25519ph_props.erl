...
-module(jose_jws_alg_ed25519ph_props).
...
jwk_jws_maps() ->
	?LET({{SK, PK}, Opts},
		{ed25519ph_keypair_gen(), opt_map()},
		begin
			JWKSigner = jose_jwk:from_okp({'Ed25519ph', SK}),
			JWKVerifier = jose_jwk:from_okp({'Ed25519ph', PK}),
...
prop_from_map_and_to_map() ->
	?FORALL(JWSMap,
		?LET({Opts, Extras},
			{opt_map(), binary_map()},
			maps:merge(maps:merge(Extras, Opts), #{ <<"alg">> => <<"Ed25519ph">> })),
		begin
