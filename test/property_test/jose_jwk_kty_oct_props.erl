...
prop_from_map_and_to_map() ->
    ?FORALL(
        ...
        begin
            ...
            JWKMap =:= element(2, jose_jwk:to_map(JWK)) andalso
                Key =:= element(2, jose_jwk:to_key(JWK)) andalso
                {not_supported, [to_public_map]} =:= Error andalso
                PublicThumbprint =:= jose_jwk:thumbprint(JWK)
        end
    ).
    ...
    ?FORALL(
        ...
        begin
            ...
            {PlainText, JWE} =:= Decrypted andalso
                {PlainText, JWE} =:= jose_jwk:block_decrypt(CompactEncrypted, JWK)
        end
    ).
