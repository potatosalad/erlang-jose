        ...
        {curve25519, [
            {
                ...
                % Output u-coordinate
                hexstr2lint("c3da55379de9c6908e94ea4df28d084f32eccf03491c71f754b4075577a28552")
            },
            {
                % Input scalar
                ...
                % Output u-coordinate
                hexstr2lint("95cbde9476e8907d7aade45cb4b873f88b595a68799fa152e6f8f7647aac7957")
            }
        ]},
        {ed25519, [
            % TEST 1
...
init_per_group(G = 'pkcs-5', Config) ->
    PBKDF1Vectors = [
        ...
        {sha, <<"password">>, <<16#78, 16#57, 16#8E, 16#5A, 16#5D, 16#63, 16#CB, 16#06>>, 1000, 16, <<
            16#DC,
            16#19,
            16#84,
            16#7E,
    ...
    PBKDF2Vectors = [
        ...
        {sha, <<"password">>, <<"salt">>, 1, 20, hex:hex_to_bin(<<"0c60c80f961f0e71f3a9b524af6012062fe037a6">>)},
        {sha, <<"password">>, <<"salt">>, 2, 20, hex:hex_to_bin(<<"ea6c014dc72d6f8ccd1ed92ace1d41f0d8de8957">>)},
        {sha, <<"password">>, <<"salt">>, 4096, 20, hex:hex_to_bin(<<"4b007901b765489abead49d926f721d065a429c1">>)},
        {sha, <<"password">>, <<"salt">>, 16777216, 20, hex:hex_to_bin(<<"eefe3d61cd4da4e4e9945b3d6ba2158c2634e984">>)},
        ...
        {sha, <<"pass\0word">>, <<"sa\0lt">>, 4096, 16, hex:hex_to_bin(<<"56fa6aa75548099dcc37d7f03425e0c3">>)},
...
emc_rsa_oaep_encrypt_and_decrypt(
    ...
    case jose_jwa_pkcs1:rsaes_oaep_encrypt(HashFun, Message, Label, Seed, RSAPublicKey) of
        ...
        EncryptError ->
            ct:fail({
                {jose_jwa_pkcs1, rsaes_oaep_encrypt, [HashFun, Message, Label, Seed, RSAPublicKey]},
                {expected, {ok, Encryption}},
                {got, EncryptError}
            })
    end,
    case jose_jwa_pkcs1:rsaes_oaep_decrypt(HashFun, Encryption, Label, RSAPrivateKey) of
        ...
        DecryptError ->
            ct:fail({
                {jose_jwa_pkcs1, rsaes_oaep_decrypt, [HashFun, Encryption, Label, RSAPrivateKey]},
                {expected, Message},
                {got, DecryptError}
            })
    end;
...
emc_rsa_pss_sign_and_verify(
    ...
    case jose_jwa_pkcs1:rsassa_pss_sign(HashFun, Message, Salt, RSAPrivateKey) of
        ...
        Other ->
            ct:fail({
                {jose_jwa_pkcs1, rsassa_pss_sign, [HashFun, Message, Salt, RSAPrivateKey]},
                {expected, {ok, Signature}},
                {got, Other}
            })
    end,
    ...
    case jose_jwa_pkcs1:rsassa_pss_verify(HashFun, Message, Signature, SaltLen, RSAPublicKey) of
        ...
        false ->
            ct:fail({
                {jose_jwa_pkcs1, rsassa_pss_verify, [HashFun, Message, Signature, SaltLen, RSAPublicKey]},
                {expected, true},
                {got, false}
            })
    end;
...
fips_aes_gcm_encrypt_and_decrypt(
    ...
    {Cipher, decrypt, {Keylen, IVlen, PTlen, AADlen, Taglen, Counts}},
    Config
) when
    bit_size(Key) =:= Keylen andalso
        bit_size(IV) =:= IVlen andalso
        bit_size(CT) =:= PTlen andalso
        bit_size(AAD) =:= AADlen andalso
        bit_size(Tag) =:= Taglen
->
...
fips_aes_gcm_encrypt_and_decrypt(
    ...
    {Cipher, encrypt, {Keylen, IVlen, PTlen, AADlen, Taglen, Counts}},
    Config
) when
    bit_size(Key) =:= Keylen andalso
        bit_size(IV) =:= IVlen andalso
        bit_size(PT) =:= PTlen andalso
        bit_size(AAD) =:= AADlen andalso
        bit_size(CT) =:= PTlen andalso
        bit_size(Tag) =:= Taglen
->
...
fips_sha3(File, Config) ->
    Options =
        case iolist_to_binary(filename:basename(File)) of
...
fips_sha3(
    ...
    {Function, Arity = 1, OutputByteLen},
    Config
) when Len rem 8 =:= 0 ->
...
fips_sha3(
    ...
    {Function, Arity = 2, OutputByteLen},
    Config
) when Len rem 8 =:= 0 ->
    InputBytes = binary:part(Msg, 0, Len div 8),
...
pbkdf1([{Hash, Password, Salt, Iterations, DerivedKeyLen, DerivedKey} | Vectors], Config) ->
    case jose_jwa_pkcs5:pbkdf1(Hash, Password, Salt, Iterations, DerivedKeyLen) of
        ...
        Other ->
            ct:fail({
                {jose_jwa_pkcs5, pbkdf1, [Hash, Password, Salt, Iterations, DerivedKeyLen]},
                {expected, {ok, DerivedKey}},
                {got, Other}
            })
    end;
pbkdf1([], _Config) ->
