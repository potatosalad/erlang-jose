...
-export([kty_hmac_from_map_and_to_map/1]).
-export([kty_hmac_block_encrypt_and_block_decrypt/1]).
-export([kty_hmac_sign_and_verify/1]).
...
groups() ->
	[
		...
		{jose_jwk_kty_hmac, [parallel], [
			kty_hmac_from_map_and_to_map,
			kty_hmac_block_encrypt_and_block_decrypt,
			kty_hmac_sign_and_verify
		]},
...
kty_hmac_from_map_and_to_map(Config) ->
	ct_property_test:quickcheck(
		jose_jwk_kty_hmac_props:prop_from_map_and_to_map(),
		Config).

kty_hmac_block_encrypt_and_block_decrypt(Config) ->
	ct_property_test:quickcheck(
		jose_jwk_kty_hmac_props:prop_block_encrypt_and_block_decrypt(),
		Config).

kty_hmac_sign_and_verify(Config) ->
	ct_property_test:quickcheck(
		jose_jwk_kty_hmac_props:prop_sign_and_verify(),
		Config).
